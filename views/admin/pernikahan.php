        ...
        <div class="p-6 border-b border-purple-50 bg-white shadow-sm">
            <form method="GET" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 items-end">
                ...
                <div>
                    ...
                    <input type="text" name="search" value="<?= htmlspecialchars($filter_search) ?>" placeholder="Nomor Surat, Nama Pria/Wanita..." class="w-full text-sm px-3 py-2 border border-purple-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 transition-shadow">
                </div>
                ...
                <div class="flex gap-2 h-[38px]">
                    ...
                    <a href="pernikahan.php" class="bg-slate-200 hover:bg-slate-300 text-slate-700 px-3 py-2 rounded-lg text-sm font-medium transition-colors shadow-sm flex items-center justify-center px-4" title="Reset Filter">
            ...
            <table class="w-full text-left border-collapse">
                ...
                <tbody class="text-sm text-slate-600 divide-y divide-purple-50">
                    ...
                    if (!empty($filter_search)) {
                        ...
                        $where_clauses[] = "(nomor_surat LIKE '%$search_clean%' OR nama_pria LIKE '%$search_clean%' OR nama_wanita LIKE '%$search_clean%')";
                    }
                    ...
                    $res = mysqli_query($conn, "SELECT * FROM surat_pernikahan $where_sql ORDER BY id DESC");
                    $no = 1;
                    if(mysqli_num_rows($res) == 0) {
                        echo '<tr><td colspan="9" class="p-8 text-center text-slate-400">Tidak ada data ditemukan.</td></tr>';
                    }
                    ...
                    <tr class="hover:bg-purple-50/50 transition-colors">
                        ...
                        <td class="p-4 flex gap-2 justify-center">
                            ...
                            <a href="pernikahan.php?action=delete&id=<?= $row['id'] ?>" onclick="confirmAction(event, this.href, 'Hapus data ini?');" class="text-pink-500 hover:text-pink-700 bg-pink-50 hover:bg-pink-100 px-2 py-1 rounded transition">Hapus</a>
                        </td>
                    </tr>
                    <?php } ?>
                </tbody>
