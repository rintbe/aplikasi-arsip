...
if($action == 'delete' && $id > 0) {
    $qFile = mysqli_query($conn, "SELECT file_pdf FROM surat_usaha WHERE id=$id");
    ...
    mysqli_query($conn, "DELETE FROM surat_usaha WHERE id=$id");
    header("Location: usaha.php?msg=deleted");
    exit;
}
...
if($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (!preg_match('/^[0-9]{16}$/', $nik)) {
        $msg = 'Gagal menyimpan: NIK Pemilik harus berupa angka dan persis 16 digit.';
    } elseif (mysqli_num_rows(mysqli_query($conn, "SELECT id FROM surat_usaha WHERE nomor_surat='$nomor_surat' AND id != $id")) > 0) {
    ...
    if(isset($_FILES['file_pdf']) && $_FILES['file_pdf']['error'] == 0 && empty($msg)) {
        ...
        if($ext == 'pdf') {
            $file_pdf = time().'_ush_'.rand(100,999).'.pdf';
    ...
    if($action == 'add' && empty($msg)) {
        $sql = "INSERT INTO surat_usaha (nomor_surat, nama_pemilik, nik, nama_usaha, bidang_usaha, alamat_usaha, tanggal_pembuatan_surat, tanggal_penerimaan_surat, file_pdf) 
                VALUES ('$nomor_surat', '$nama_pemilik', '$nik', '$nama_usaha', '$bidang_usaha', '$alamat_usaha', '$tanggal_pembuatan', '$tanggal_penerimaan', '$file_pdf')";
        ...
        header("Location: usaha.php?msg=added");
        exit;
    } elseif($action == 'edit' && empty($msg)) {
        $setFile = $file_pdf ? ", file_pdf='$file_pdf'" : "";
        $sql = "UPDATE surat_usaha SET nomor_surat='$nomor_surat', nama_pemilik='$nama_pemilik', nik='$nik', 
                nama_usaha='$nama_usaha', bidang_usaha='$bidang_usaha', alamat_usaha='$alamat_usaha', 
                tanggal_pembuatan_surat='$tanggal_pembuatan', tanggal_penerimaan_surat='$tanggal_penerimaan' $setFile WHERE id=$id";
        mysqli_query($conn, $sql);
        header("Location: usaha.php?msg=updated");
        exit;
    }
}
...
if ($action == 'add' && $pengajuan_id > 0) {
    ...
    if ($p_data) {
        ...
        if($action == 'edit' && $id > 0) {
            $res = mysqli_query($conn, "SELECT * FROM surat_usaha WHERE id=$id");
    ...
    <div class="bg-white rounded-2xl p-8 border border-purple-100 shadow-sm">
        <div class="flex justify-between items-center mb-6">
            <h2 class="text-xl font-bold text-slate-800"><?= $action == 'add' ? 'Tambah' : 'Edit' ?> Surat Usaha</h2>
            <a href="usaha.php" class="text-slate-500 hover:text-slate-700 bg-slate-100 px-4 py-2 rounded-lg text-sm font-medium transition">Batal</a>
        </div>
        ...
        <form method="POST" enctype="multipart/form-data" class="space-y-6">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    <label class="block text-sm font-medium text-slate-700 mb-2">Nama Pemilik</label>
                    <input type="text" name="nama_pemilik" value="<?= $row['nama_pemilik'] ?? '' ?>" required class="w-full px-4 py-2 rounded-lg border border-purple-200 focus:outline-none focus:ring-2 focus:ring-purple-500">
                </div>
                <div>
                    <label class="block text-sm font-medium text-slate-700 mb-2">NIK Pemilik</label>
                ...
                <div>
                    ...
                    <input type="text" name="nama_usaha" value="<?= $row['nama_usaha'] ?? '' ?>" required class="w-full px-4 py-2 rounded-lg border border-purple-200 focus:outline-none focus:ring-2 focus:ring-purple-500">
                </div>
                <div>
                    <label class="block text-sm font-medium text-slate-700 mb-2">Bidang Usaha</label>
                    <input type="text" name="bidang_usaha" value="<?= $row['bidang_usaha'] ?? '' ?>" required class="w-full px-4 py-2 rounded-lg border border-purple-200 focus:outline-none focus:ring-2 focus:ring-purple-500">
                </div>
                <div>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="alamat_usaha" required rows="3" class="w-full px-4 py-2 rounded-lg border border-purple-200 focus:outline-none focus:ring-2 focus:ring-purple-500"><?= $row['alamat_usaha'] ?? '' ?></textarea>
                </div>
                <div>
    ...
    <div class="bg-white rounded-2xl border border-purple-100 shadow-sm overflow-hidden">
        <div class="p-6 border-b border-purple-50 flex flex-col md:flex-row justify-between items-center bg-gray-50/50 gap-4">
            <h2 class="text-xl font-bold text-slate-800">Daftar Surat Usaha</h2>
            <div class="flex items-center gap-4 w-full md:w-auto">
                <a href="export_excel.php?type=usaha" class="flex-shrink-0 bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-lg text-sm font-medium flex items-center transition-colors shadow-sm">
                    <i class="fa-solid fa-file-excel mr-2"></i> Ekspor
                </a>
                <a href="usaha.php?action=add" class="flex-shrink-0 bg-purple-600 hover:bg-purple-700 text-white px-4 py-2 rounded-lg text-sm font-medium flex items-center transition-colors shadow-sm">
        ...
        <div class="p-6 border-b border-purple-50 bg-white shadow-sm">
            <form method="GET" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 items-end">
                ...
                <div>
                    ...
                    <input type="text" name="search" value="<?= htmlspecialchars($filter_search) ?>" placeholder="Nomor Surat, Nama Usaha, Pemilik..." class="w-full text-sm px-3 py-2 border border-purple-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 transition-shadow">
                </div>
                ...
                <div class="flex gap-2 h-[38px]">
                    ...
                    <a href="usaha.php" class="bg-slate-200 hover:bg-slate-300 text-slate-700 px-3 py-2 rounded-lg text-sm font-medium transition-colors shadow-sm flex items-center justify-center px-4" title="Reset Filter">
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-purple-50 text-purple-700 font-medium text-sm">
                        ...
                        <th class="p-4 border-b border-purple-100">Nama Pemilik</th>
                        <th class="p-4 border-b border-purple-100">Nama Usaha</th>
                        <th class="p-4 border-b border-purple-100">Bidang</th>
                ...
                <tbody class="text-sm text-slate-600 divide-y divide-purple-50">
                    ...
                    if (!empty($filter_search)) {
                        ...
                        $where_clauses[] = "(nomor_surat LIKE '%$search_clean%' OR nama_pemilik LIKE '%$search_clean%' OR nama_usaha LIKE '%$search_clean%')";
                    }
                    ...
                    $res = mysqli_query($conn, "SELECT * FROM surat_usaha $where_sql ORDER BY id DESC");
                    $no = 1;
                    ...
                    <tr class="hover:bg-purple-50/50 transition-colors">
                        ...
                        <td class="p-4"><?= $row['nama_pemilik'] ?></td>
                        <td class="p-4"><?= $row['nama_usaha'] ?></td>
                        <td class="p-4"><?= $row['bidang_usaha'] ?></td>
                        ...
                        <td class="p-4 flex gap-2 justify-center">
                            <a href="usaha.php?action=edit&id=<?= $row['id'] ?>" class="text-sky-500 hover:text-sky-700 bg-sky-50 hover:bg-sky-100 px-2 py-1 rounded transition">Edit</a>
                            <a href="usaha.php?action=delete&id=<?= $row['id'] ?>" onclick="confirmAction(event, this.href, 'Hapus data ini?');" class="text-pink-500 hover:text-pink-700 bg-pink-50 hover:bg-pink-100 px-2 py-1 rounded transition">Hapus</a>
                        </td>
                    </tr>
                    <?php } ?>
                </tbody>
